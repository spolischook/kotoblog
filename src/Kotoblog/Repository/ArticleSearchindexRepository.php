...
class ArticleSearchindexRepository implements SearchableInterface
{
    /** @var  EntityManager */
    protected $em;
    ...
    public function __construct(EntityManager $em, CacheProvider $cache, TextHelper $textHelper, $searchWeight, $searchMinWordLength)
    {
        $this->em = $em;
    ...
    public function search($string)
    {
        ...
        foreach ($words as $word) {
            ...
            $queryBuilder
                ->select('s')
                ->from('Kotoblog\Entity\ArticleSearchindex', 's')
                ->where('s.word = :word')
                ->setParameter(':word', $word)
            ;

            $articlesSearchIndexArray = $queryBuilder->getQuery()->execute();

            /** @var ArticleSearchindex $articlesSearchIndex */
            foreach ($articlesSearchIndexArray as $articlesSearchIndex) {
                @$preResults[$articlesSearchIndex->getArticleId()] += $articlesSearchIndex->getWeight();
            }
        }
        ...
        foreach ($preResults as $articleId => $weight) {
            $searchResults[] = $this->em->getRepository('Kotoblog\Entity\Article')->find($articleId);
        }
